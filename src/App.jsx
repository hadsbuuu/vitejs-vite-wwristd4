...
const SAMPLE_PROJECTS = [
  {
    id: genId(),
    name: 'Tech Blog',
    weeklyGoals: '2 blogs, 15 backlinks, interlinking',
    priority: 'high',
    color: '#F59E0B',
  },
  {
    id: genId(),
    name: 'Health Blog',
    weeklyGoals: '1 blog, 10 backlinks',
    priority: 'medium',
    color: '#3B82F6',
  },
];
...
function balanceTasks(tasks) {
  ...
  tasks.forEach((t) => {
    if (t.day >= 0 && t.day < 7) dayLoad[t.day]++;
  });
...
export default function App() {
  ...
  const loadData = async () => {
    ...
    if (proj && proj.length > 0) {
      ...
      if (tsk) setTasks(tsk);
      const { data: hab } = await supabase.from('habits').select('*');
  ...
  const todayTasks = useMemo(
    () => tasks.filter((t) => t.day === TODAY_IDX),
    [tasks]
  );
  const missedTasks = useMemo(
    () => tasks.filter((t) => t.day < TODAY_IDX && !t.done),
    [tasks]
  );
  ...
  const moveTask = async (id, day) => {
    await supabase.from('tasks').update({ day }).eq('id', id);
  ...
  const addProject = async () => {
    ...
    const p = {
      id: genId(),
      ...newProject,
      color: ['#F59E0B', '#3B82F6', '#22C55E', '#EF4444', '#8B5CF6'][
    ...
    const parsed = parseGoals(
      newProject.weeklyGoals,
      p.id,
      p.name,
      newProject.priority
    );
    const balanced = balanceTasks([...tasks, ...parsed]);
    const newTasks = balanced.filter((t) => t.projectId === p.id);
  ...
  const autoReschedule = async () => {
    const dayLoad = Array(7).fill(0);
    tasks.forEach((x) => {
      if (x.day >= 0 && x.day < 7) dayLoad[x.day]++;
    });
    ...
    const changed = rescheduled.filter(
      (t, i) => t.day !== tasks[i].day
    );
    for (const t of changed) {
      await supabase.from('tasks').update({ day: t.day }).eq('id', t.id);
    }
    setTasks(rescheduled);
  };

  // --- END SUPABASE UPDATED FUNCTIONS ---
...
Keep breakdown to max 7 days. Be concrete and actionable.`,
          messages: [
            {
              role: 'user',
              content: `Suggest an optimized weekly plan for: "${aiInput}"`,
            },
          ],
        }),
      });
      ...
      setAiSuggestion({
        suggestion: "Couldn't reach AI — try: 5 per day for 4 days",
        breakdown: [],
        reasoning: 'Balanced distribution over weekdays.',
      });
    }
    setAiLoading(false);
  };
  ...
  const applyAiSuggestion = async () => {
    ...
    const newTasks = aiSuggestion.breakdown.map((b, i) => ({
      ...
      projectName: projects[0]?.name || 'Manual',
      priority: 'medium',
      title: b.task,
      day: Math.min(i, 6),
      done: false,
  ...
  if (focusMode) {
    return (
      ...
      >
        ...
        >
          Focus Mode · {FULL_DAYS[TODAY_IDX]}
        </p>
        <h2
          style={{
            fontSize: 22,
        ...
        <div style={{ width: '100%', maxWidth: 480 }}>
          {todayTasks.length === 0 ? (
            <p style={{ textAlign: 'center', color: '#9CA3AF', fontSize: 16 }}>
    ...
    return (
      ...
      >
        ...
        <div style={{ textAlign: 'center', maxWidth: 520 }}>
          ...
          >
            Task {justStartIdx + 1} of {todo.length}
          </p>
          {current ? (
            <>
              <div
              ...
              <button
                onClick={() => {
                  ...
                  setJustStartIdx((i) => Math.min(i + 1, todo.length - 1));
                }}
                style={{
                  background: '#22C55E',
                  border: 'none',
              ...
              <p style={{ marginTop: 16, color: '#475569', fontSize: 13 }}>
                tap when complete to see next task
              </p>
            </>
          ) : (
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 64, marginBottom: 16 }}>🎉</div>
              <h2 style={{ color: '#F1F5F9', fontSize: 28, marginBottom: 8 }}>
                All done!
              </h2>
        ...
        {navItems.map((n) => (
          <button
            ...
            style={{
              ...
              borderRadius: 8,
              border: 'none',
              background: view === n.id ? '#F3F4F6' : 'transparent',
              cursor: 'pointer',
              fontSize: 14,
              color: view === n.id ? '#111827' : '#6B7280',
      ...
      <div style={{ flex: 1, padding: '2rem', overflowY: 'auto' }}>
          ...
          <div>
            ...
            >
              <div>
                ...
                <p
                  style={{ color: '#9CA3AF', fontSize: 14, margin: '4px 0 0' }}
                >
                  {todayTasks.filter((t) => t.done).length} of{' '}
                  {todayTasks.length} tasks done
            ...
            >
              {[
                ...
                {
                  label: 'Done',
                  value: todayTasks.filter((t) => t.done).length,
                  sub: 'today',
                },
                ...
                >
                  <button
                        ...
                        hs.map((x) =>
                          x.id === h.id
                            ? {
                                ...
                                streak: !x.completedToday
                                  ? x.streak + 1
                                  : Math.max(0, x.streak - 1),
                              }
                            : x
                        )
                      )
                    ...
                    style={{
                      ...
                      height: 24,
                      borderRadius: '50%',
                      border: h.completedToday ? 'none' : '2px solid #D1D5DB',
                      background: h.completedToday ? '#22C55E' : 'transparent',
                      cursor: 'pointer',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                  ...
                  <span
                    style={{
                      flex: 1,
                      fontSize: 14,
                      color: h.completedToday ? '#9CA3AF' : '#111827',
          ...
          <div>
            ...
            >
              ...
              {missedTasks.length > 0 && (
                ...
                >
                  ↺ Auto-reschedule {missedTasks.length} missed
                </button>
              )}
            </div>
            <div
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      border: `1px solid ${isToday ? '#BFDBFE' : '#E5E7EB'}`,
                      borderRadius: 12,
                      padding: '12px 8px',
                      minHeight: 200,
                    ...
                    >
                      ...
                      <span
                        style={{
                          display: 'block',
                          fontSize: 10,
                          color: dayTasks.length >= 6 ? '#EF4444' : '#9CA3AF',
                        }}
                      >
                        {dayTasks.length}/6
                      </span>
                      ...
                      .map((t) => (
                        <div
                          ...
                          style={{
                            ...
                            background: t.done
                              ? '#F9FAFB'
                              : COLORS[t.priority]?.bg || '#F9FAFB',
                            border: `1px solid ${
                              t.done
                                ? '#E5E7EB'
                                : COLORS[t.priority]?.border || '#E5E7EB'
                            }`,
                            marginBottom: 4,
                            cursor: 'pointer',
                            fontSize: 11,
                        ...
                        >
                          <div
                            style={{
                              ...
                              height: 6,
                              borderRadius: '50%',
                              background: COLORS[t.priority]?.dot,
        ...
        {view === 'projects' && (
          <div>
            ...
            {showAddProject && (
              ...
              >
                <h3
                  style={{ margin: '0 0 1rem', fontSize: 16, fontWeight: 600 }}
                >
                  New Project
                </h3>
                <div
                ...
                >
                  <input
                    ...
                    placeholder="Project name (e.g., Tech Blog)"
                    style={{
                      padding: '10px 14px',
                      border: '1px solid #E5E7EB',
                      borderRadius: 8,
                  ...
                  />
                  <textarea
                    value={newProject.weeklyGoals}
                    onChange={(e) =>
                      setNewProject((p) => ({
                        ...p,
                        weeklyGoals: e.target.value,
                      }))
                    }
                    placeholder="Weekly goals (e.g., 2 blogs, 20 backlinks, interlinking)"
                    rows={3}
                    style={{
                      padding: '10px 14px',
                      border: '1px solid #E5E7EB',
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewProject((p) => ({ ...p, priority: e.target.value }))
                    }
                    style={{
                      padding: '10px 14px',
                      border: '1px solid #E5E7EB',
                      ...
                      >
                        <span
                          style={{
                            ...
                            padding: '3px 10px',
                            borderRadius: 999,
                            background: COLORS[p.priority]?.bg,
                            color: COLORS[p.priority]?.text,
                            border: `1px solid ${COLORS[p.priority]?.border}`,
                          }}
                        >
                          {p.priority}
                        </span>
            ...
            >
              <div style={{ display: 'flex', gap: 10, marginBottom: '1rem' }}>
                <input
                  ...
                  placeholder="Add a daily habit (press Enter)..."
                  style={{
                    flex: 1,
                    padding: '10px 14px',
                    border: '1px solid #E5E7EB',
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div
                      style={{
                        fontSize: 12,
                        color: h.streak > 0 ? '#F59E0B' : '#9CA3AF',
                      }}
                    >
        ...
        {view === 'stats' && (
          <div>
            ...
            >
              <h3 style={{ margin: '0 0 1rem', fontSize: 15, fontWeight: 600 }}>
                Daily Load
              </h3>
              <div
                style={{
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <div
                          style={{
                            width: '100%',
                            height: `${(done / 6) * 80}px`,
                            background: i === TODAY_IDX ? '#3B82F6' : '#22C55E',
                            borderRadius: '4px 4px 0 0',
                            position: 'absolute',
                            bottom: 0,
                          }}
                      ...
                      <span
                        style={{
                          fontSize: 10,
                          color: i === TODAY_IDX ? '#3B82F6' : '#9CA3AF',
                        }}
                      >
                        {d}
                      </span>
            ...
            <div
              style={{
                background: completionRate >= 70 ? '#F0FDF4' : '#FEF2F2',
                border: `1px solid ${
                  completionRate >= 70 ? '#BBF7D0' : '#FECACA'
                }`,
                borderRadius: 14,
                padding: '1.25rem',
              }}
            >
              <div
                style={{
                  fontSize: 14,
                  fontWeight: 600,
                  color: completionRate >= 70 ? '#166534' : '#991B1B',
                  marginBottom: 4,
                }}
              >
                {completionRate >= 70
              ...
              >
                {completionRate >= 70
                  ? "You're completing most tasks. Keep the momentum!"
                  : completionRate >= 40
                  ? "You're completing some tasks but have room to improve. Consider reducing your weekly goals."
                  : 'Most tasks are incomplete. Start with just 1 task per day and build up gradually.'}
              </div>
            </div>
          </div>
        )}
        ...
        {view === 'ai' && (
          <div>
            ...
            <p
              style={{ color: '#9CA3AF', fontSize: 14, marginBottom: '1.5rem' }}
            >
              Describe a vague goal and get an optimized weekly plan.
            </p>
            <div
              style={{
                background: '#fff',
            ...
            >
              <div style={{ display: 'flex', gap: 10, marginBottom: '1rem' }}>
                <input
                  ...
                  placeholder="e.g., '50 backlinks', '3 blogs this week', 'increase traffic'"
                  style={{
                    flex: 1,
                    padding: '12px 16px',
                    border: '1px solid #E5E7EB',
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                onClick={async () => {
                  ...
                  await supabase.from('tasks').update({ title, day, priority }).eq('id', editTask.id);
                  setTasks((ts) =>
                    ts.map((t) =>
                      t.id === editTask.id ? { ...t, title, day, priority } : t
                    )
                  );
                  setEditTask(null);
                }}
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 12, color: '#9CA3AF' }}>
          {task.projectName} · {FULL_DAYS[task.day]}
        </div>
      </div>
      <span
        style={{
